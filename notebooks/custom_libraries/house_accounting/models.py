...
class Cashflow(Base):
    ...
    _unique_check_dict = {
        "main_category": {"class": MainCategory, "field": MainCategory.category, "field_name": "category"},
        "sub_category": {"class": SubCategory, "field": SubCategory.category, "field_name": "category"},
        "time_category": {"class": TimeCategory, "field": TimeCategory.category, "field_name": "category"},
    ...
    def project_element(session, input_obj, target_class):
        ...
        if isinstance(sel_filter, list):
            existent_elements = session.query(chck_dict["class"]).filter(chck_dict["field"].in_(sel_filter)).all()
            if len(existent_elements) == len(sel_filter):
                return existent_elements
            else:
                new_list = []
                new_list.extend(existent_elements)
                allo = [getattr(jjj, chck_dict["field_name"]) for jjj in existent_elements]
            ...
            existent_elements = session.query(chck_dict["class"]).filter(chck_dict["field"] == sel_filter).all()
        ...
        for kkk, vvv in Cashflow._unique_check_dict.items():
            if kkk in kwargs:
                try:
                    ...
                    print(f"problem during projection of item {kkk} with content {kwargs[kkk]}")
                    raise excp
